...
// tersendiri buat menu IGD (App.tsx) supaya bisa dikembangkan lebih
// lanjut (triase, tindakan emergensi) tanpa ikut mengubah Pendaftaran.
// Satu-satunya beda dari RegistrasiView saat ini: fetchPatients selalu
// kirim `igd=1` ke /api/registrasi/list, jadi daftarnya dibalik ke HANYA
...
export const IGDKView: React.FC = () => {
  ...
  // Fetch data from API — mode dikunci ke 'registrasi-awal' & igd=1
  // fixed: HANYA kunjungan kd_poli='IGDK' (lihat komentar
  // getRegistrasiList di backend/main.go). Tanpa tab Registrasi
  // Awal/Rujukan Internal Poli — IGD cukup satu daftar saja.
  const fetchPatients = React.useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      let url = `/api/registrasi/list?mode=registrasi-awal&igd=1`;

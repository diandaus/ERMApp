...
// (inventory/InventorySuplier.java) — CRUD master pemasok obat/alkes/BHP.
// Kode boleh diganti (mengikuti Java), tapi backend menolak kalau kode
// masih dipakai pembelian.kode_suplier. Lihat backend/apotek_suplier_handler.go.
// ============================================================================
...
const emptyForm = (): Suplier => ({
  kode_suplier: '', nama_suplier: '', alamat: '', kota: '', no_telp: '', nama_bank: '', rekening: '',
});

export const ApotekSuplierView: React.FC = () => {
  const [items, setItems] = React.useState<Suplier[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const [searchText, setSearchText] = React.useState('');

  const [showModal, setShowModal] = React.useState(false);
  const [form, setForm] = React.useState<Suplier>(emptyForm());
  ...
  const fetchItems = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/apotek/suplier${searchText ? `?search=${encodeURIComponent(searchText)}` : ''}`);
      if (!res.ok) throw new Error('Gagal mengambil data suplier');
  ...
  const openEditModal = (item: Suplier) => {
    setForm(item);
    setEditingKode(item.kode_suplier);
    setShowModal(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editingKode ? `/api/apotek/suplier/${encodeURIComponent(editingKode)}` : '/api/apotek/suplier';
  ...
  const handleDelete = async (item: Suplier) => {
    const confirm = await Swal.fire({
      title: 'Hapus Suplier?',
      text: `${item.kode_suplier} — ${item.nama_suplier}`,
      icon: 'warning',
    ...
    setDeletingKode(item.kode_suplier);
    try {
      const res = await fetch(`/api/apotek/suplier/${encodeURIComponent(item.kode_suplier)}`, { method: 'DELETE' });
      const data = await res.json();
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead style={{ position: 'sticky', top: 0, background: '#f3f4f6', zIndex: 1 }}>
            <tr>
              ...
              <th style={{ padding: 8, textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Nama Suplier</th>
              ...
              <th style={{ padding: 8, textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Bank</th>
              <th style={{ padding: 8, textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Rekening</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8} style={{ padding: 24, textAlign: 'center', color: '#6b7280' }}>Belum ada data suplier</td></tr>
            ) : (
              items.map((item, index) => (
                <tr key={item.kode_suplier} style={{ background: index % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', color: '#374151' }}>{item.kode_suplier}</td>
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', color: '#111827' }}>{item.nama_suplier}</td>
                  ...
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', color: '#374151' }}>{item.nama_bank}</td>
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', color: '#374151' }}>{item.rekening}</td>
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', textAlign: 'center' }}>
                    <div style={{ display: 'inline-flex', gap: 6 }}>
                      ...
                      <button
                        ...
                        disabled={deletingKode === item.kode_suplier}
                        style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #dc2626', background: '#ffffff', color: '#dc2626', cursor: deletingKode === item.kode_suplier ? 'not-allowed' : 'pointer', fontSize: 11, fontWeight: 500 }}
                      >
                        {deletingKode === item.kode_suplier ? 'Menghapus...' : 'Hapus'}
                      </button>
                    </div>
                  </td>
                </tr>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>{editingKode ? 'Edit Suplier' : 'Tambah Suplier'}</span>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Field label="Kode *">
                <input required style={inputStyle} value={form.kode_suplier} onChange={(e) => setForm((p) => ({ ...p, kode_suplier: e.target.value }))} />
              </Field>
              <Field label="Nama Suplier *">
                <input required style={inputStyle} value={form.nama_suplier} onChange={(e) => setForm((p) => ({ ...p, nama_suplier: e.target.value }))} />
              </Field>
              <Field label="Alamat">
              ...
              <Field label="Nama Bank">
                <input style={inputStyle} value={form.nama_bank} onChange={(e) => setForm((p) => ({ ...p, nama_bank: e.target.value }))} />
              </Field>
              <Field label="No. Rekening">
                <input style={inputStyle} value={form.rekening} onChange={(e) => setForm((p) => ({ ...p, rekening: e.target.value }))} />
              </Field>

...
// (inventory/DlgGolongan.java) — CRUD master golongan barang, tabel yang
// sama dipakai referensi dropdown "Golongan" di Data Barang. Kode boleh
// diganti (mengikuti Java), tapi backend menolak kalau kode masih
// dipakai databarang. Lihat backend/apotek_golongan_handler.go.
// ============================================================================
...
type GolonganBarang = { kode: string; nama: string };
const emptyForm = (): GolonganBarang => ({ kode: '', nama: '' });

export const ApotekGolonganView: React.FC = () => {
  const [items, setItems] = React.useState<GolonganBarang[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const [searchText, setSearchText] = React.useState('');

  const [showModal, setShowModal] = React.useState(false);
  const [form, setForm] = React.useState<GolonganBarang>(emptyForm());
  ...
  const fetchItems = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/apotek/golongan${searchText ? `?search=${encodeURIComponent(searchText)}` : ''}`);
      if (!res.ok) throw new Error('Gagal mengambil data golongan barang');
  ...
  const openEditModal = (item: GolonganBarang) => {
    setForm(item);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editingKode ? `/api/apotek/golongan/${encodeURIComponent(editingKode)}` : '/api/apotek/golongan';
  ...
  const handleDelete = async (item: GolonganBarang) => {
    ...
    setDeletingKode(item.kode);
    try {
      const res = await fetch(`/api/apotek/golongan/${encodeURIComponent(item.kode)}`, { method: 'DELETE' });
      const data = await res.json();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <input
          type="text"
          placeholder="Cari Kode / Nama Golongan..."
          value={searchText}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead style={{ position: 'sticky', top: 0, background: '#f3f4f6', zIndex: 1 }}>
            <tr>
              ...
              <th style={{ padding: 8, textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Nama Golongan</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} style={{ padding: 24, textAlign: 'center', color: '#6b7280' }}>Belum ada data golongan barang</td></tr>
            ) : (
              items.map((item, index) => (
                <tr key={item.kode} style={{ background: index % 2 === 0 ? '#ffffff' : '#f9fafb' }}>
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', color: '#374151' }}>{item.kode}</td>
                  ...
                  <td style={{ padding: '6px 8px', borderBottom: '1px solid #e5e7eb', textAlign: 'center' }}>
                    <div style={{ display: 'inline-flex', gap: 6 }}>
                      ...
                      <button
                        ...
                        disabled={deletingKode === item.kode}
                        style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #dc2626', background: '#ffffff', color: '#dc2626', cursor: deletingKode === item.kode ? 'not-allowed' : 'pointer', fontSize: 11, fontWeight: 500 }}
                      >
                        {deletingKode === item.kode ? 'Menghapus...' : 'Hapus'}
                      </button>
                    </div>
                  </td>
                </tr>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>{editingKode ? 'Edit Golongan Barang' : 'Tambah Golongan Barang'}</span>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Field label="Kode Golongan *">
                <input required style={inputStyle} value={form.kode} onChange={(e) => setForm((p) => ({ ...p, kode: e.target.value }))} />
              </Field>

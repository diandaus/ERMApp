...
import { ModalHasilLabPK } from '../components/ModalHasilLabPK';

// LaboratoriumPKView — worklist departemen Laboratorium PK lintas pasien.
// Padanan DlgCariPermintaanLab.java (Khanza Desktop): 2 tab terpisah "Rawat
// Jalan"/"Rawat Inap" (permintaan_lab.status='ralan'/'ranap' — Java yang
// dikirim hardcode 'ralan' saja, ditambah tab ranap di sini spy konsisten
// dgn Radiologi.tsx), kolom Ruang beda sumber per tab (Poliklinik utk
// ralan, kamar+bangsal terbaru utk ranap — dihitung backend). Tombol Input
// Hasil (ModalHasilLabPK) per baris. Permintaan baru dibuat dari layar
// pasien (LabTab.tsx > ModalInputLab.tsx), bukan dari modul ini. Baca
// daftar dari GET /api/lab-pk/list?rawat=ralan|ranap.
...
type LaboratoriumPKViewProps = { user?: { nip?: string } | null };

export const LaboratoriumPKView: React.FC<LaboratoriumPKViewProps> = ({ user }) => {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/lab-pk/list?${params}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Gagal memuat daftar permintaan lab PK');
  ...
  const handleSubmitSampel = async () => {
    ...
    try {
      const res = await fetch(`/api/lab-pk/sampel/${encodeURIComponent(sampelRow.noorder)}`, {
        method: 'POST',
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={14} style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>Tidak ada permintaan lab PK pada rentang ini</td></tr>
            ) : (
              rows.map((row) => (
                <tr key={row.noorder}>

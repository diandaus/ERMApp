...
import { ModalHasilLabPA } from '../components/ModalHasilLabPA';

// LaboratoriumPAView — worklist departemen Laboratorium PA lintas pasien.
// Padanan PERSIS pola LaboratoriumPK.tsx (2 tab Rawat Jalan/Rawat Inap,
// tombol "+Sampel" per baris, tombol "+Hasil" membuka ModalHasilLabPA) tapi
// sumbernya permintaan_labpa (bukan permintaan_lab) — kolom tabel mengikuti
// header Khanza Desktop yg dikirim user (termasuk kolom khas PA: Tgl.Bahan,
// Diperoleh Dengan, Lokasi Jaringan, Diawetkan Dengan, riwayat PA
// sebelumnya). Permintaan baru dibuat dari layar pasien (LabTab.tsx >
// ModalInputLab.tsx), bukan dari modul ini.
...
type QueueRow = {
  ...
  tgl_sampel: string; jam_sampel: string;
  tgl_hasil: string; jam_hasil: string;
  kd_dokter: string; nm_dokter: string;
  poli_registrasi: string; informasi_tambahan: string; diagnosa_klinis: string;
  kd_pj: string; png_jawab: string;
  tgl_pengambilan_bahan: string; diperoleh_dengan: string; lokasi_jaringan: string; diawetkan_dengan: string;
  pernah_dilakukan_di: string; tanggal_pa_sebelumnya: string; nomor_pa_sebelumnya: string; diagnosa_pa_sebelumnya: string;
  rawat: string; pemeriksaan: string;
};
...
type LaboratoriumPAViewProps = { user?: { nip?: string } | null };

export const LaboratoriumPAView: React.FC<LaboratoriumPAViewProps> = ({ user }) => {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/lab-pa/list?${params}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Gagal memuat daftar permintaan lab PA');
  ...
  // Badge "belum ada sampel" di switch tab — sama pola LaboratoriumPK.tsx.
  ...
  const handleSubmitSampel = async () => {
    ...
    try {
      const res = await fetch(`/api/lab-pa/sampel/${encodeURIComponent(sampelRow.noorder)}`, {
        method: 'POST',
  ...
  const columns = [
    ...
    'Dokter Perujuk', 'Poli Registrasi', 'Informasi Tambahan', 'Diagnosis Klinis',
    'Jenis Bayar', 'Tgl.Bahan', 'Diperoleh Dengan', 'Lokasi Jaringan', 'Diawetkan Dengan',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, minHeight: 0, height: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap', marginLeft: 'auto' }}>
          ...
          <div style={{ minWidth: 300, position: 'relative' }}>
            ...
            <input
              ...
              placeholder="No.Permintaan / No.Rawat / No.RM / Nama Pasien / Dokter..."
      ...
      <div style={{ flex: 1, minHeight: 0, borderRadius: 8, border: '1px solid #e5e7eb', overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={columns.length} style={{ padding: 20, textAlign: 'center', color: '#6b7280' }}>Memuat...</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={columns.length} style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>Tidak ada permintaan lab PA pada rentang ini</td></tr>
            ) : (
              rows.map((row) => (
                <tr key={row.noorder}>
                  ...
                  <td style={{ ...TD, whiteSpace: 'nowrap', maxWidth: 220 }}>
                  ...
                  <td style={{ ...TD, whiteSpace: 'nowrap' }}>{row.poli_registrasi || '-'}</td>
                  <td style={TD}>{row.informasi_tambahan || '-'}</td>
                  <td style={TD}>{row.diagnosa_klinis || '-'}</td>
                  <td style={{ ...TD, whiteSpace: 'nowrap' }}>{row.png_jawab || '-'}</td>
                  <td style={{ ...TD, whiteSpace: 'nowrap' }}>{row.tgl_pengambilan_bahan || '-'}</td>
                  <td style={TD}>{row.diperoleh_dengan || '-'}</td>
                  <td style={TD}>{row.lokasi_jaringan || '-'}</td>
                  <td style={TD}>{row.diawetkan_dengan || '-'}</td>
                  <td style={TD}>{row.pernah_dilakukan_di || '-'}</td>
                  <td style={{ ...TD, whiteSpace: 'nowrap' }}>{row.tanggal_pa_sebelumnya || '-'}</td>
                  <td style={{ ...TD, whiteSpace: 'nowrap' }}>{row.nomor_pa_sebelumnya || '-'}</td>
                  <td style={TD}>{row.diagnosa_pa_sebelumnya || '-'}</td>
                  <td style={TD}>{row.pemeriksaan || '-'}</td>
                </tr>
              ))
            )}
          </tbody>

...
// (inventory/DlgKategori.java) — CRUD master kategori barang, tabel yang
// sama dipakai referensi dropdown "Kategori" di Data Barang. Kode boleh
// diganti (mengikuti Java), tapi backend menolak kalau kode masih
// dipakai databarang. Lihat backend/apotek_kategori_handler.go.
// ============================================================================
...
type KategoriBarang = { kode: string; nama: string };
const emptyForm = (): KategoriBarang => ({ kode: '', nama: '' });

export const ApotekKategoriView: React.FC = () => {
  const [items, setItems] = React.useState<KategoriBarang[]>([]);
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const [searchText, setSearchText] = React.useState('');

  const [showModal, setShowModal] = React.useState(false);
  const [form, setForm] = React.useState<KategoriBarang>(emptyForm());
  ...
  const fetchItems = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/apotek/kategori${searchText ? `?search=${encodeURIComponent(searchText)}` : ''}`);
      if (!res.ok) throw new Error('Gagal mengambil data kategori barang');
  ...
  const openEditModal = (item: KategoriBarang) => {
    setForm(item);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = editingKode ? `/api/apotek/kategori/${encodeURIComponent(editingKode)}` : '/api/apotek/kategori';
  ...
  const handleDelete = async (item: KategoriBarang) => {
    ...
    try {
      const res = await fetch(`/api/apotek/kategori/${encodeURIComponent(item.kode)}`, { method: 'DELETE' });
      const data = await res.json();
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead style={{ position: 'sticky', top: 0, background: '#f3f4f6', zIndex: 1 }}>
            <tr>
              ...
              <th style={{ padding: 8, textAlign: 'left', borderBottom: '2px solid #e5e7eb' }}>Nama Kategori</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={3} style={{ padding: 24, textAlign: 'center', color: '#6b7280' }}>Belum ada data kategori barang</td></tr>
            ) : (
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: '#111827' }}>{editingKode ? 'Edit Kategori Barang' : 'Tambah Kategori Barang'}</span>

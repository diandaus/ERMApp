...
export const ModalBilling: React.FC<ModalBillingProps> = ({ noRawat, namaPasien, jaminan, onClose }) => {
  ...
  const buildBillingPdf = async (): Promise<Uint8Array> => {
    ...
    const col = { uraian: margin, pemisah: margin + 100, biaya: margin + 260, jumlah: margin + 380, total: margin + 440 };
    ...
    rows.forEach((row) => {
      ...
      if (isTextOnlyRow) {
        if (namaTrim.startsWith('Total')) {
          ...
          // — rata kanan sbg satu string utuh ke margin kanan halaman,
          // tanpa dipecah ke kolom pemisah/uraian spt baris label:value.

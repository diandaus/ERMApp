...
interface ModalRegistrasiIGDProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

// ModalRegistrasiIGD.tsx — copy dari ModalRegistrasi.tsx, dipakai IGDK.tsx
// (menu IGD) supaya bisa dikembangkan lebih lanjut (mis. default Poli
// terkunci ke IGDK) tanpa ikut mengubah modal Pendaftaran biasa.
export const ModalRegistrasiIGD: React.FC<ModalRegistrasiIGDProps> = ({ isOpen, onClose, onSuccess }) => {
  // Form input states
  ...
  // Unit dikunci ke IGDK — modal ini cuma dipakai dari menu IGD, jadi
  // field pilih poli dihilangkan dari form, tidak perlu staf pilih manual.
  const [selectedPoli] = React.useState<string>('IGDK');
  ...
  React.useEffect(() => {
    if (isOpen) {
      const loadMasterData = async () => {
        try {
          const [dokterRes, penjabRes] = await Promise.all([
            fetch('/api/pendaftaran/dokter'),
            fetch('/api/pendaftaran/penjab')
          ]);

          if (dokterRes.ok && penjabRes.ok) {
            const [dokterData, penjabData] = await Promise.all([
              dokterRes.json(),
              penjabRes.json()
            ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!noRkmMedis || !selectedPoli || !selectedDokter || !kdPj) {
      setError('Lengkapi No. RM, Dokter, dan Cara Bayar.');
      return;
    }
